...
export function ProjectionWorkspace() {
  ...
  return (
    <section className="absolute inset-x-4 bottom-4 top-20 flex flex-col gap-3">
      <div className="glass pointer-events-auto flex items-center justify-between rounded-xl px-4 py-2.5">
        <div className="flex min-w-0 items-center gap-3">
          <Grid2X2 size={18} className="shrink-0 text-cyan-300" />
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-100">
              三面投影图与三维模型联动
              <span className="rounded bg-cyan-400/10 px-2 py-0.5 text-[10px] font-medium text-cyan-200">组合状态</span>
            </div>
            <p className="truncate text-[11px] text-slate-400">{stageLabel(progress)} · 当前：{selectedName}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-cyan-300/25 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-100 transition hover:bg-cyan-400/18"
          >
            {playing ? <Pause size={14} /> : progress < 1 ? <Play size={14} /> : <RotateCcw size={14} />}
            <span>{playing ? '暂停' : progress < 1 ? '继续生成' : '重新生成投影'}</span>
          </button>
          <button
            type="button"
            onClick={() => setWorkspaceMode('model')}
            className="flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300 transition hover:bg-white/5 hover:text-white"
          >
            <Box size={14} /> 返回三维模型
          </button>
        </div>
        ...
        {PANELS.map((panel) => {
          ...
          return (
            <article key={panel.id} className="projection-panel relative min-h-0 overflow-hidden rounded-xl border border-cyan-300/15 bg-[#071827]/90">
              <div className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-start justify-between bg-gradient-to-b from-slate-950/85 to-transparent px-3 py-2.5">
                <div>
                  <h2 className="text-xs font-semibold tracking-wide text-cyan-50">{panel.title}</h2>
                  <p className="mt-0.5 text-[9px] uppercase tracking-[.16em] text-cyan-300/55">{panel.note}</p>
                </div>
                {panel.id !== 'isometric' && <span className="rounded border border-cyan-300/15 px-1.5 py-0.5 text-[9px] text-cyan-200/60">ORTHOGRAPHIC</span>}
              </div>
      ...
      <div className="glass flex h-8 items-center gap-3 rounded-lg px-3 text-[10px] text-slate-400">
